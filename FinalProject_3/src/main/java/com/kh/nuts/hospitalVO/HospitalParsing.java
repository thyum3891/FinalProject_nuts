package com.kh.nuts.hospitalVO;
...
//			int count = Integer.parseInt(hs.totalCount());
	...
	public List<HospitalVO> parsing(int page, int num) {
		String parsingXml = apiXML(num,page);
		List<HospitalVO> resultList = new ArrayList<HospitalVO>();
		...
		for (int i = 1; i < xmlList.size(); i++) {
			...
			HospitalVO result = new HospitalVO(rnum, publisher, creator, creator, title, subjectCategory, reference,
